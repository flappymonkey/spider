...
class SmzdmPipeline(object):
    ...
    def from_crawler(cls, crawler):
        ...
        return cls(settings.get('MONGODB_SERVER', 'localhost'), settings.get('MONGODB_PORT', 27017),
            settings.get('MONGODB_DB', 'scrapy'), settings.get('MONGODB_COLLECTION', None),settings.get('MONGODB_TEMP_COLLECTION', None),
    ...
    def close_spider(self,spider):
        ...
        if spider.scrapy_mode == 2 or spider.scrapy_mode == 1:
            ...
            try:
                ...
                if key in get_result_dict:
                    ...
                    if self.uniq_key is None:
                        result = self.temp_collection.insert(dict(item), safe=self.safe)
                    else:
                        result = self.temp_collection.update({self.uniq_key: item[self.uniq_key] }, { '$set': dict(item) },upsert=True, safe=self.safe)
